...
try:
    ...
    print("  python setup/bootstrap_install.py")
    print()
    print("Or install PyYAML manually first:")
    print("  pip install pyyaml")
    print("  python setup/manage_versions.py install-yaml")
    print()
    exit(1)
...
class PackageVersionManager:
    def __init__(self, config_file="package_config.yaml"):
        # Handle relative path from setup directory
        if not Path(config_file).is_absolute():
            # If we're in setup directory, look in parent directory
            if Path.cwd().name == "setup":
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "show":
            ...
            print(
                "Usage: python setup/manage_versions.py [show|install|install-yaml|clone|install-deps|run-script|install-package <name>|env]"
            )
            print("Options:")
